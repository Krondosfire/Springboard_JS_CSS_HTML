document.addEventListener("DOMContentLoaded", function ()
{
	...
	function loadNotes() {
		...
		for (const note of notes) {
			...
			noteElement.style.backgroundColor = note.color; // Sets the note's background color using the last selected note color.
			noteContainer.appendChild(noteElement);  // Appends it to the note container element as its child.
		}
	}

	loadNotes(); // Loads the notes from the local storage.
	...
	function addNewNote ()
	{
		...
		noteIdCounter++; // Increments the counter since the ID is used for this note.
		const notes = readNotes();
		notes.push({ id: noteIdCounter, content: content, color: noteColor });
		saveNotes(notes);
	...
	colorForm.addEventListener("submit", function (event)
	{
		...
		localStorage.setItem('noteColor', noteColor); // Updates the note color in the local storage.
	});
	...
	document.addEventListener("dblclick", function (event)
	{
		if (event.target.classList.contains("note"))
		{
			...
			for ( let i = 0; i < notes.length; i++)
			{
				if (notes[i].id === id)
				{
					notes.splice(i, 1); // Removes the note from the array.
					
				}
			}
			saveNotes(notes); // Saves the updated notes array to local storage.
		}
	});
	...
	noteContainer.addEventListener("blur", function (event)
	{
		if (event.target.classList.contains("note"))
		{
			...
			for (let i = 0; i < notes.length; i++)
			{
				if (notes[i].id === id)
				{
					notes[i].content = event.target.value; // Updates the note content.
				}
			}
			saveNotes(notes); // Saves the updated notes array to local storage.
		}
